...
**CLAIM_DECISION_APPROVED / REFUNDED / REFUSED_FINAL** — `POST /api/admin/claims/[id]/arbitrate {decision: approve|refuse_final, reason?}` (admin) → `arbitrateClaim` (approve ⇒ `triggerClaimRefund` ⇒ engine `executeRefund`, itself gated `REFUNDS_ENABLED`) → audit → `refunded = result.refund?.state==='refunded'` ⇒ decision `refunded` (amount = the engine's **result.refund.amountCents**, carried on the CAS-won refunded outcome; HISTORICAL, before the 2026-09-06 hotfix: claim.requestedAmountCents) · approve without succeeded refund (pending/failed/gated) ⇒ `approved` (no amount) · refuse_final ⇒ `refusalEmailKind(claim)`: `refused_final` when the restaurant itself refused on record, otherwise `refused_by_grubano` (both under trigger `claim_decision_refused_final`).

**ROUND 13 (H02, H03)** — every claim sender takes `claimsOpen: isClaimsEnabled()`, read at send time: while claims are closed nothing is sent and the skip is traced (`EmailLog` `(non envoyé : claims_disabled)`). A refuse_final is `refused_final` (« Refus confirmé ») only when the restaurant itself refused on record, otherwise `refused_by_grubano` (same trigger `claim_decision_refused_final`); `refunded` only on the engine's CAS-won `refunded` result, with the engine's amount.

**CLAIM_CLOSURE_REFUND / CLAIM_CLOSED_BY_SUPPORT** (closure notices, H05–H08) — send sites, and only these: `POST /api/admin/claims/[id]/resolve-stuck` (after the declaration and its audit) → `sendClaimClosureEmail({claimId, claimsOpen})`; `POST /api/admin/claims/[id]/reconcile` on outcome `refunded` → evidence `{stripe_read, amount}` when Stripe's refund object was read; `POST /api/admin/claims/[id]/attribute` (row and adopt branches) on an observed `refunded` commit; `POST /api/admin/claims/[id]/closure-notice` (per-claim resend, empty body; a refunded claim is re-read at Stripe first). The `claim_decision_refunded` decision e-mail is also sent by `POST /api/claims` on the `auto_small` path when `autoResolveSmallClaim` returns `refunded` (`sendClaimDecisionEmail`, behind `CLAIM_AUTO_RESOLVE_ENABLED`, off in the beta; T4's refunded compare-and-set wrote the closure record first) — round 13 W8 correction. The sender checks, in order: claim → closure kind → THIS build's closure record (`EmailDispatch` `claim_closure_record` / `claim:<id>`, never an audit row) → claims open → refusal kinds delegate to the decision sender → refunded: bound row proven, one binder, Stripe evidence → recipient → `sendTransactional` under `claim_decision_refunded` or `claim_closed_by_support`. No other path sends a closure notice (H09).
