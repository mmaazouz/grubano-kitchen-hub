...
import { Truck, Check, Clock, ShoppingBasket, FileDown, Star, ChevronRight, Minus, Plus, Share2, X, Sparkles, AlertCircle } from "lucide-react";
...
// Catalog: each product, with availability per supplier
const catalog = [
  {
...
function SuppliersPage() {
  const [mode, setMode] = useState<"choice" | "catalog" | "partner" | "self">("choice");
  ...
  if (mode === "choice") {
    return (
      <FeaturePage title="Commander" subtitle="Comment souhaitez-vous restocker ?">
        <button onClick={() => setMode("catalog")} className="mb-3 w-full overflow-hidden rounded-2xl border border-primary/30 bg-gradient-to-br from-accent to-card p-4 text-left active:scale-[0.99] transition">
          ...
          {partners.map((p) => (
            <button key={p.name} onClick={() => setSupplier(p)} className="w-full rounded-2xl border border-border bg-card p-3.5 text-left active:scale-[0.99] transition">
              <div className="flex items-start gap-3">
                <div className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-accent text-primary font-bold">{p.name[0]}</div>
                <div className="flex-1">
...
// ---------- Catalog (multi-supplier comparison) ----------
function CatalogView({ onBack }: { onBack: () => void }) {
  ...
  if (sent) {
    return (
      <FeaturePage title="Commandes envoyées" subtitle={`${supplierCount} fournisseurs`}>
  ...
  return (
    <FeaturePage title="Catalogue" subtitle="Meilleur prix par produit" badge={`€${total.toFixed(0)}`}>
        ...
        {catalog.map((item) => (
          <Card key={item.name} className="!p-0 overflow-hidden">
            <div className="flex items-center justify-between border-b border-border bg-muted/30 px-3.5 py-2.5">
              ...
              <span className="text-[10px] font-bold text-muted-foreground">{item.offers.filter(o => o.stock !== "out").length}/{item.offers.length} dispo</span>
            </div>
              ...
              {item.offers.map((o) => {
                ...
                return (
                  <button
                    ...
                    disabled={isOut}
                    className={`flex w-full items-center justify-between px-3.5 py-2.5 text-left transition ${isSelected ? "bg-accent" : ""} ${isOut ? "opacity-40 cursor-not-allowed" : "active:bg-muted/40"}`}
                  >
                    <div className="flex items-center gap-2.5">
                      <div className={`grid h-5 w-5 place-items-center rounded-full border-2 ${isSelected ? "border-primary bg-primary" : "border-border"}`}>
                        {isSelected && <Check size={10} className="text-primary-foreground" />}
                      </div>
                      <div>
                        <div className="flex items-center gap-1.5">
                          <p className="text-[12px] font-bold">{o.supplier}</p>
                          {"badge" in o && o.badge && <span className="rounded-full bg-success/15 px-1.5 py-0.5 text-[8px] font-bold text-success">{o.badge}</span>}
                        </div>
                    ...
                    <div className="text-right">
                      <p className="text-sm font-bold">€{o.price.toFixed(2)}<span className="text-[9px] font-normal text-muted-foreground">/{item.unit}</span></p>
                      {!isOut && <p className="text-[10px] text-muted-foreground">= €{(o.price * item.needed).toFixed(2)}</p>}
                    </div>
                  </button>
                );
              })}
              {item.offers.every(o => o.stock === "out") && (
                <div className="flex items-center gap-2 px-3.5 py-2 text-[11px] text-warning bg-warning/5">
                  <AlertCircle size={12} /> <button className="font-bold underline">Suggérer une alternative IA</button>
                </div>
              )}
            </div>
          </Card>
      ...
      <div className="sticky bottom-24 mt-5 rounded-2xl border border-border bg-card p-4 shadow-lg">
        <div className="mb-3 flex items-center justify-between">
          <div>
            <p className="text-[10px] uppercase tracking-wider text-muted-foreground">Total · {supplierCount} fournisseurs</p>
