...
import {
  Camera, Sparkles, Check, Users, Clock, AlertTriangle, ChevronRight, QrCode, Plus, CalendarDays, Euro, Filter, ShieldCheck,
} from "lucide-react";
...
type Reservation = {
  name: string; table: string; time: string; end: string; guests: number; allergy: string | null;
  arrived: boolean; paid: number; type: "quick" | "standard" | "full"; status: "confirmed" | "arrived" | "overrun";
};
...
function TablesPage() {
  const [tab, setTab] = useState<"list" | "calendar" | "plan" | "setup">("list");
  return (
  ...
  return (
    <>
      ...
      <div className="mb-3 flex items-center gap-2 overflow-x-auto pb-1">
        ...
        {([["all", "Toutes"], ["arrived", "Arrivés"], ["allergy", "Allergies"], ["deposit", "Acompte"]] as const).map(([k, l]) => (
          <button
            key={k}
            onClick={() => setFilter(k)}
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {r.paid > 0 && <span className="rounded-full bg-accent px-1.5 py-0.5 text-[9px] font-bold text-primary">€{r.paid} acompte</span>}
                  <span className={`rounded-full px-1.5 py-0.5 text-[9px] font-bold uppercase ${
                    r.type === "quick" ? "bg-muted text-muted-foreground" : r.type === "standard" ? "bg-warning/20 text-warning" : "bg-primary/15 text-primary"
                  }`}>
                    {r.type === "quick" ? "Rapide" : r.type === "standard" ? "Standard" : "Complet"}
                  </span>
                </div>
                <p className="mt-0.5 text-[11px] text-muted-foreground">{r.table} · {r.guests} couverts · libère {r.end}</p>
                {r.allergy && (
              ...
              {!r.arrived && (
                <button className="rounded-lg bg-primary px-2.5 py-1.5 text-[10px] font-semibold text-primary-foreground">Arrivée</button>
              )}
            </div>
          </div>
        ))}
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl border border-border bg-card p-4">
        ...
        <p className="mt-1 text-[11px] text-muted-foreground">Pré-autorisation carte bancaire requise à la réservation.</p>
      ...
      <div className="rounded-2xl border border-border bg-card p-4">
        ...
        <ul className="mt-3 space-y-2 text-[12px]">
          <li className="flex items-center justify-between"><span>🥪 Rapide (sandwich, bowl)</span><span className="font-bold">25-35 min</span></li>
          <li className="flex items-center justify-between"><span>🍝 Standard (plat principal)</span><span className="font-bold">45-60 min</span></li>
          <li className="flex items-center justify-between"><span>🍽️ Complet (entrée + plat + dessert)</span><span className="font-bold">75-90 min</span></li>
        </ul>
        <p className="mt-3 text-[10px] text-muted-foreground">+15 min de tampon ajouté automatiquement avant la prochaine réservation.</p>
      </div>

      <button className="flex w-full items-center justify-between rounded-2xl border border-border bg-card p-3 text-sm">
        <span className="flex items-center gap-2 font-semibold"><Camera size={14} className="text-primary" /> Refaire le plan de salle (IA)</span>
        <ChevronRight size={14} className="text-muted-foreground" />
      </button>
      <button className="flex w-full items-center justify-between rounded-2xl border border-border bg-card p-3 text-sm">
        <span className="flex items-center gap-2 font-semibold"><Plus size={14} className="text-primary" /> Ajouter une table</span>
        <ChevronRight size={14} className="text-muted-foreground" />
      </button>
    </div>
  );
