...
| P3 Annulation resto payée | `p3-annulation-resto-payee.test.ts` | 18 | 16 | 0 | 2 | **Ré-photographié vague 4 (P0-08)** : l'absence de remboursement automatique est désormais l'état VOULU (Q3) — annuler une commande PAYÉE crée une demande d'arbitrage SYSTÈME dans la MÊME transaction (motif `system_order_cancelled`, montant intégral) + email honnête localisé ; NON payée = chemin historique byte-identique. Plus aucun FAIL-ATTENDU. |
| P4 Annulation client | `p4-annulation-client.test.ts` | 7 | 2 | 4 | 1 | Annulation client **inexistante** (aucune route ; PATCH status → 403 pour un consumer). *(Ligne recomptée vague 4 — l'ancienne « 7 | 3 | 3 | 1 » ne correspondait pas au fichier.)* |
| P5 Refund humain | `p5-refund-humain.test.ts` | 8 | 6 | 0 | 2 | Modale `/finance` inerte **par construction** (bouton `disabled` en dur) ; le rail owner `POST /api/orders/[id]/refund` fonctionne, lui, indépendamment de la surface. *(Ligne recomptée vague 4 — le test ajouté par P0-26 n'avait pas été reporté ici.)* |
...
| P9 Avis / ★ seedée | `p9-avis-etoile-seedee.test.ts` | 12 | 7 | 4 | 1 | `orderId` fantôme stocké verbatim, aucun check rôle/commande (auto-avis possible) ; `Restaurant.rating` seedé jamais recalculé (documenté « choix produit différé » dans le code). **Ré-photographié vague 4 (V4-2)** : la fiche publique ne sert PLUS la ★ stockée sans avis réel (gate `lib/review-stats`) — le FAIL-ATTENDU « fiche sert la colonne seedée » est INVERSÉ ; la colonne stockée reste non recalculée (défaut toujours ouvert). |
...
**Totaux : 90 PASS-ACTUEL · 34 FAIL-ATTENDU · 22 NON-TESTABLE** (à la création — 146 tests ; après vague 1 — P8 ACT 1 + P10 : 148 = 94 · 32 · 22 ; après vague 2 — P8 ACT 2 (P0-29) : 149 = 98 · 29 · 22 ; après vague 4 — P3 (P0-08, durci en revue) puis P9 fiche publique (V4-2) : **154 tests MESURÉS = 105 · 27 · 22** — recomptage vitest complet en revue adversariale vague 4 ; les totaux intermédiaires ci-avant portaient l'écart hérité des lignes P4/P5, corrigées dans le tableau).
