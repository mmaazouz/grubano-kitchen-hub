...
// refund of the same order and SUCCEED it at Stripe — the customer's money is gone — while
// `triggerClaimRefund` still returns `{ state: 'failed', error: 'resume_mismatch' }`, because it
// did not settle THIS claim (lib/claims.ts, REFUND IDENTITY BINDING). Rendering that as
// « le remboursement a ÉCHOUÉ : aucun argent n'est parti » tells the admin the opposite of the
// truth, and the obvious next action — issue the refund again — pays the customer twice.
...
describe('RESUME-FIRST — money moved, and the admin must never be told otherwise', () => {
  ...
  it('the French copy tells the admin money DID leave and forbids a second refund', () => {
    const fr = JSON.parse(fs.readFileSync('messages/fr.json', 'utf8'))
    const copy: string = fr.claims.admin.approvedResumeMismatch
    expect(copy).toMatch(/EST parti/)                 // money left
    expect(copy).toMatch(/Ne relancez aucun remboursement/) // and: do not pay again
