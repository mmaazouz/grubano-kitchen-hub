...
# Inbox page ID: 396fd2c9-8146-8110-8942-d2db957bae9e  (📥 Rapports CC → Agent 0 — CANAL ACTIF v2 : page FIXE unique, append en bas, le plus récent en bas. NE PLUS versionner — anciennes boîtes v1..v79 = archives READ-ONLY)
...
### Schema changes
⚠️ Vérifié le 2026-09-22 : `scripts/server/prisma-push.sh` pointe la PRODUCTION (`~/grubano.com`), n'est PAS déployé (le workflow ne livre que `scripts/server/*.js`) et lance un `db push` GLOBAL qui a déjà refusé sans `--accept-data-loss`. Ne l'utilisez pas pour une colonne additive. Procédure en vigueur (Phase 1, puis D′) : opérateur JS additif `ALTER TABLE … ADD COLUMN IF NOT EXISTS` avec sauvegarde vérifiée (`scripts/server/phase1-staging-migrate.js`, `scripts/server/dprime-staging-migrate.js`), schéma poussé AVANT le code, puis régénération du client Prisma dans le nodevenv (`phase1-regen-client.js` / `dprime-regen-client.js`), jamais `--accept-data-loss`. Toujours `./node_modules/.bin/prisma` (never global `prisma`).
Rappel historique (production, à ne pas rejouer sans décision fondateur) :
```bash
...
| `scripts/server/prisma-push.sh` | Historique (production, non déployé, `db push` global). Depuis Phase 1 : opérateurs additifs `phase1-staging-migrate.js` / `dprime-staging-migrate.js` + `*-regen-client.js` (voir §7). |
...
Voir §7 « Schema changes » : opérateur additif + regen client, schéma AVANT code (`prisma-push.sh` = historique production, non déployé sur staging).
