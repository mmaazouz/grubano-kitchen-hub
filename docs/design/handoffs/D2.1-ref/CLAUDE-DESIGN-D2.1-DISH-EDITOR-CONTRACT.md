...
Titres, libellés champs, pied, suppression, photo : chaînes du handoff verbatim. **Ajouts design (aides passives)** : « JPG, PNG ou WebP · 8 Mo max » (tuile) · « Supérieur à 0 € » (prix) · « Facultatif — nombre entier » (calories) · « Sans sélection, le client verra “Informations allergènes non renseignées par le restaurant”. » (allergènes) — **quote exacte de la phrase consommateur** rendue par la modale plat de `/eat/r/[id]` quand la liste d'allergènes est vide (clé `eat.restaurant.allergensNone`). Cette aide est une PROMESSE au restaurateur : elle doit rester alignée mot pour mot sur la réalité consommateur (arbitrage fondateur 2026-09-01, voir les ARBITRAGES du §10). ⚠️ `dish-editor.html:240` conserve l'ANCIENNE citation « Information non renseignée par le restaurant » : la référence Design est obsolète sur cette seule ligne. Elle n'est volontairement pas retouchée — un artefact Claude Design n'est pas réécrit après coup — la divergence est déclarée ici et le produit fait foi.
...
**ARBITRAGES FONDATEUR (2026-09-01)** : *(a)* **tuile photo REMPLIE** — aucun pixel de référence n'existe pour cet état (la référence ne dessine que la drop-zone VIDE), alors que c'est ce que l'opérateur voit à chaque édition d'un plat qui a une photo. Statut : **EXPLICITLY ACCEPTED — CURRENT PRODUCT BEHAVIOR**. Aucun traitement Design n'est inventé, D2.1 n'est pas bloqué par cette référence manquante ; la tuile vide/fallback et tous les états contractuellement référencés restent soumis à la comparaison Design normale. *(b)* **allergènes vides** — la promesse de §9 doit égaler la réalité consommateur : côté `/eat/r/[id]`, une liste vide n'efface PAS le bloc allergènes, elle affiche l'avertissement explicite « Informations allergènes non renseignées par le restaurant. En cas d'allergie, contactez l'établissement avant de commander. » Le silence n'est pas une option : il laisserait déduire « aucun allergène ». L'état renseigné (ex. Lactose, Céleri) est **inchangé**.
