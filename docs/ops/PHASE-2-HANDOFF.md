...
> **AMENDMENT 2026-09-05 (Phase 2 FINAL PREFLIGHT — ledger reconciliation + internal auth forensics; overrides earlier sentences where they conflict).** v3 PASSED on the server (REFUNDS_ENABLED runtime FALSE = 403 gated before auth, reload proven, waitlist IN, tips FALSE, ALERT_EMAIL m.maazouz@grubano.com accepted, no unexpected refund, GR-N5TSM0 measured) except the internal HTTP ledger route → 401. **Loader model corrected (measured from the deploy):** the deployed Passenger entry is Next's generated `.next/standalone/server.js` (the repo-root `server.js` is NOT shipped — `CLAUDE.md §9` is wrong, ticket T-40), so env is loaded by `@next/env` (dotenv): `.env.production.local` › `.env.local` › `.env.production` › `.env`, first file wins, **pre-existing process.env (hosting/cPanel/Passenger injection) is never overridden**, `KEY = v`/leading spaces/`export` ARE loaded, last occurrence in a file wins. The v3 "non-loadable line" diagnosis was unfounded (canonical lines are read identically by every loader; no side effect). **Two verdicts, kept separate:** FINANCIAL LEDGER RECONCILIATION is run DIRECTLY by the operator v4 with `lib/ledger-check-core.js` (the route's logic extracted unchanged, shipped to `deploy-temp/lib/`, tested 11/12/…) against DB + Stripe TEST — read-only; INTERNAL LEDGER HTTP AUTH is diagnosed by the value-free provenance preamble injected by `scripts/fix-server.js` into the deployed entry (`~/.grubano/env-provenance.json`: presentBeforeEnvLoad / definedIn / equalProcessVsFiles / effectiveSource — booleans and file names only) plus `.github/workflows/internal-token-probe.yml` (status only: GitHub secret == staging runtime token?). Decision matrix + rotation procedure: `docs/ops/RUNTIME-SECRET-SOURCE-MATRIX.md`. Route auth unchanged (tests: wrong/missing/Bearer/empty-env → 401, no bypass, no leak). Same founder command. No refund, no Stripe LIVE, no schema.

> **AMENDMENT 2026-09-05 (v5 — FINAL FINANCIAL RECONCILIATION, plumbing fix only).** v4 was NOT MEASURED, not a mismatch: the operator had no DATABASE_URL for Prisma (it stopped populating process.env) and `stripe` is absent from the Next standalone runtime (bundled into route chunks). v5 (`scripts/server/reconcile-helpers.js`, tested) loads env through `@next/env` (the app loader), hands Prisma the URL explicitly, and uses a READ-ONLY Stripe REST client (GET only, api.stripe.com pinned) when the SDK is absent; `lib/ledger-check-core.js` unchanged. Windows A = 7 d, B = since 2026-08-28; every `not_in_stripe_window` ecart is proven WINDOW_EDGE_ONLY or stays TRUE_FINANCIAL_MISMATCH; a window passes only with `ok` or with proven edges AND exact sums after exclusion; unchecked refunds = FAIL. Negative controls proven locally (extra line / wrong amount / missing PI / duplicate refund → FAIL). Web-root exposure: secret classes all 404 (no confirmed leak), source classes 200 = P1 pre-production, ticket T-41 + `WEB-ROOT-HARDENING-HANDOFF.md`; `.htaccess` untouched; token/TIPS/ALERT_EMAIL hosting provenance documented only (`RUNTIME-SECRET-SOURCE-MATRIX.md` §8). Same founder command; push `[no-restart]`.
