...
| `CLAIMS_SURFACE_ENABLED` | `lib/claim-flags.ts` | **D′ L1 (2026-09-22, spec v2 §3) — flag PRODUIT de la bêta.** La fonctionnalité réclamations EXISTE : listes client/resto/admin, réponses, décisions, contestations, e-mails pré-argent, réclamations système. Seule la chaîne exacte `true` l’active (`TRUE`/`1`/`` = OFF). Sous `SURFACE=true` le bail legacy (`CLAIMS_ENABLED`+`CLAIMS_WINDOW_UNTIL`) est INERTE. N’ouvre ni auto-approve, ni auto-resolve, ni ghost, ni le moteur de remboursement (S-13). Kill-switch = `SURFACE=false` **et** `INTAKE=false` **et** bail legacy absent : `POST /api/claims` 403 `{gated:true}`, `GET /api/admin/claims` **scindé** (`enabled:false`, listes workflow vides, **liste argent toujours renvoyée**), avis post-argent et clôtures toujours envoyables. Exige la table `Claim`. |
| `CLAIMS_INTAKE_ENABLED` | `lib/claim-flags.ts` | **D′ L1 — flag PRODUIT.** Un client peut DÉPOSER une nouvelle réclamation. Exige `CLAIMS_SURFACE_ENABLED=true` (ERREUR `check-flags` sinon — n’ouvre rien seul). `SURFACE=true`+`INTAKE=false` = intake suspendu : `POST /api/claims` 403 `{gated:false, enabled:true, intakeOpen:false, reason:'intake_closed'}` (une sonde lit UNKNOWN, jamais CLOSED), `GET /api/claims?orderId` overlay `canClaim:false / reason:'intake_closed'` (sauf `not_owner`), tout le reste (historique, contest, resto, admin, réclamations système) inchangé (S-23). |
| `CLAIMS_ENABLED` | `lib/claim-flags.ts` (ré-exporté par `lib/claims.ts`) | **Depuis D′ L1 : bail de RÉPÉTITION uniquement (Mode A 2026-09-15→18, Mode B 2026-09-22 `dab754d`, tous deux CLOS).** Sans flag produit, le bail ouvre surface ET intake ensemble (S-12) ; sous `CLAIMS_SURFACE_ENABLED=true` il est INERTE (WARNING `check-flags`). Les opérateurs Mode A/B et `phase2-refund-gate window` REFUSENT (en nommant le flag) si `CLAIMS_SURFACE_ENABLED` ou `CLAIMS_INTAKE_ENABLED` vaut `true` (S-14). Réclamations client (⚠️ exige la table `Claim` en base — cf. harnais P10)  **T-53 (2026-09-10) : le drapeau seul n’autorise plus RIEN.** La surface réclamations exige AUSSI `CLAIMS_WINDOW_UNTIL` (ISO, futur, ≤ 60 min), revérifié par l’application à chaque appel : une fenêtre meurt de vieillesse même après un SIGKILL, un crash hôte ou un redémarrage. Ce bail n’accorde AUCUNE autorité de remboursement. |
