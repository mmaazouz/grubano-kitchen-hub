...
| `REFUNDS_ENABLED` | **Aucun remboursement in-app possible**, même admin (les 3 routes → 403). Seul recours : dashboard Stripe. | Rouvre `POST /api/orders/[id]/refund` (admin Grubano uniquement) + le moteur `refunds/run` + ⚠️ le refund sur **accept d'une réclamation par le restaurateur**. |
