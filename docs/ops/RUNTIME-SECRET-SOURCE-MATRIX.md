...
3. GitHub : mettre à jour le secret repo `INTERNAL_CRON_TOKEN` via `gh secret set INTERNAL_CRON_TOKEN < fichier-temporaire` exécuté **sur le serveur** (le fondateur, avec `gh` authentifié) — ou, si `gh` n'y est pas installé, coller la valeur dans l'UI GitHub Secrets **sans jamais la transmettre à l'agent**. Vérifier par `internal-token-probe.yml` (dispatch → `GITHUB SECRET == STAGING RUNTIME TOKEN = YES`).
