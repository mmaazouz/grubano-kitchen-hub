...
IMPLEMENTATION NOTE (certification audit c32d8d3, P1): a claim in this state is never settled by reconcileClaimForRefund — a stale, duplicate or out-of-order 'succeeded' delivery, and a recovery pass whose Stripe read preceded the reversal, return 'stripe_reverted' and write nothing (G13 note); the declaration exit is unchanged.
...
IMPLEMENTATION NOTE (certification audit c32d8d3, P1): the token half of (f) is pinned — status still 'refunding' with another attempt's token → attempt_superseded, no write, executeRefund not called (tests/claims-r13-trigger.test.ts); break/restore control R21.
IMPLEMENTATION NOTE (targeted re-audit of 2466e03, P3): the status half of (f) is pinned as well — status changed with the token kept → attempt_superseded, no write, executeRefund not called (tests/claims-r13-trigger.test.ts); break/restore control R30.
IMPLEMENTATION NOTE (targeted re-audit of d9fb194, P1): the loader's read of the order's rows is a stamped read too — a row carrying this claim's identity in read.facts.rows (or read.rows on no_charge) takes ownRowExists right after the loader, before (b'), (c) and (e'), exactly as (a) would, so an own stamped row that (a) did not see (a stalled attempt's insert, read skew) invalidates the attempt on every branch; the (e') changed_during_read branch stays as a guard. Pinned by the window-revert and hold read-skew fixtures in tests/claims-t49-round13-alerts.test.ts; break/restore control R31.
IMPLEMENTATION NOTE (targeted re-audit of 75f1601, P1 — the class closed at its last link): the d9fb194 note above overstated its reach. « On every branch » did not hold: loadOrderMoneyFacts reads the order's rows at its step 2 and can still fail afterwards (the royalty read, the PaymentIntent retrieve, the refund list — transient or list_over_cap —, a row's Stripe truth, a binder or stamped-claim read), returning a variant that carries no rows; and « Pinned by the window-revert and hold read-skew fixtures » covered only the readable arm, not read.rows on no_charge. Every non-engine exit of triggerClaimRefund (revertPreImage, safetyHold, the lock/AWAITING proof write) now takes a stamped read immediately before its own write: a row found → ownRowExists; a failed read → no hold or proof text (the hold and the proof revert with safety_check_unreadable, a transient cause the next attempt re-derives) and the outcome-unknown title; none → no row carrying this claim's identity exists at that read, refund rows being never deleted. The loader check stays as the early exit; its no_charge arm is now defence in depth behind the hold's own read, pinned by the stamped-read count. Between an unreadable loader return and an exit's own read only pure template code runs (safetyHoldText, noChargeClause), so the catch's attempt_crashed title rests on the same reads; ownRowExists, the (f) read, T4 (engineCalled true) and the catch are unchanged. The (e') changed_during_read branch is unreachable behind the loader check and kept as a guard. The only residual is a row inserted between that read and the compare-and-set, the declared C11 / A-S33 class. Break/restore controls R33 (revert), R34 (hold), R35 (proof write), R36 (the loader check's no_charge arm).
IMPLEMENTATION NOTE (targeted re-audit of 68621aa, P1 and P2): two corrections to the 75f1601 note above. (1) The (e') park was a non-engine exit without its own read: enterFinancialVerification attributed the order's refunds from (a) and the loader's step-2 rows, so a row stamped claim:<id> inserted after step 2 whose Stripe refund the list already tagged was parked as « n’est rattaché ni à l’identité de cette réclamation ». It now takes the same stamped read before enterFinancialVerification: a row found → ownRowExists; a failed read → revertPreImage('safety_check_unreadable'); none → park. Every non-engine exit of triggerClaimRefund — revertPreImage, safetyHold, the lock/AWAITING proof write and the park — now reads the rows stamped for the claim immediately before its own compare-and-set. (2) A failed own read in safetyHold, the proof write or the park writes no hold, proof or park text and reverts to the pre-image; the revert takes its own stamped read, and the title follows that read: the frozen title when it returns none (the absence is then read), the outcome-unknown title when it fails too. The note above said the outcome-unknown title is kept, which holds only when both reads fail. Pinned by FAILED_OWN_READ (the (b') no-charge hold, the (b') list_over_cap hold, the (c) hold, the (e') lock proof and the (e') park, each with the exit's read failing and the revert's read returning none, and with both failing) and by INSERTED_AFTER_LOADER, which now includes the park. Break/restore controls R37 and R38 (the failed-read branch of the hold and of the proof write falling through to the write), R39 and R40 (the same branches deleted), R41 (the park's own read removed) and R42 (the park's failed-read branch falling through).
...
IMPLEMENTATION NOTE (certification audit c32d8d3, P1): « CARD … Always mounted » is pinned structurally in tests/claims-t49-routes.test.ts — AdminFinancialVerification mounted once, on its own line, directly under the island's ToastProvider, with no flag guard — with negative controls for the inline &&, ternary, flag-call, multi-line &&, early-return and removal shapes; break/restore control R23.
IMPLEMENTATION NOTE (targeted re-audit of 2466e03, P1 and its P2 sibling): the source-shape pin could not see a flag gate on an ancestor of the card, nor an early exit written another way — the previous note's « no flag guard » covered only the card's own lines. The unconditional mount is now pinned by BEHAVIOUR in tests/claims-admin-page-fv-mount.test.ts: the page's server component is called with the claims flag off and then on, and its returned element tree is walked — flag off: exactly one AdminFinancialVerification inside the ToastProvider island, no AdminClaimsArbitration, no redirect; flag on: both. Break/restore controls R23, R28 (the island gated on the flag) and R29 (an early exit written `claimsOpen === false`).
IMPLEMENTATION NOTE (targeted re-audit of d9fb194, P3): the behavioural pin now RENDERS the returned tree (react-dom/server with marker components): flag off — the card's marker exactly once, no arbitration marker, and no hidden attribute, hiding style or hiding class anywhere on the page; flag on — both markers. Its negative controls are built from the page's REAL returned tree: the island replaced by false, a wrapper component rendering nothing, the section hidden by attribute, class or style, and an early exit. Break/restore control R32 (the section hidden on the flag). Nothing outside the page gates /admin/claims at HEAD (middleware.ts, next.config.js and app/[locale]/layout.tsx carry no claims rule; there is no admin layout) — read, not rendered.
IMPLEMENTATION NOTE (targeted re-audit of 75f1601, P3): the d9fb194 note above overstated the pin. « No hidden attribute, hiding style or hiding class anywhere on the page » meant the hidden attribute, display:none / visibility:hidden and the classes hidden / invisible / sr-only; its early-exit control is a synthetic null tree, not built from the page. The rendered pin now also rejects aria-hidden, inert, opacity 0, zero width or height, clip or a negative left offset, and details / dialog / template elements, with negative controls built from the page's real tree for aria-hidden, inert, opacity 0, zero height and a details wrapper. Hiding shapes outside this list (a portal, a class or rule a stylesheet hides) are not checked.
...
IMPLEMENTATION NOTE (certification audit c32d8d3, P1 and its P2 sibling): reconcileClaimForRefund's succeeded branch refuses a claim carrying a reversal marker (isStripeReverted) — reason 'stripe_reverted', no write, console [MONEY REVIEW] stripe_reverted_not_settled — because a reversal leaves our row 'succeeded' by design, so our own row cannot prove the refund held. This closes the webhook path (a stale, duplicate or out-of-order 'succeeded' delivery after STRIPE_REVERTED) and the sweep interleaving (the Stripe read preceding the reversal). Pinned in tests/webhook-refund-reconciliation.test.ts and tests/claims-t49-recovery.test.ts (J-M48 (c')); break/restore control R20.
...
IMPLEMENTATION NOTE (certification audit c32d8d3, P1): the TITLE above is kept only where no rail payment for this claim can have happened. When engineCalled is true (resume_mismatch, identity_unverified, engine_own_row, engine_failed, attempt_crashed after the engine call) or the cause is own_row_exists (a row stamped for this claim already exists), the title is « Tentative de remboursement sans issue établie — preuve requise avant toute décision » (claimBlockedTitle). Kind, dedupe key, causes and facts are unchanged; break/restore control R24.
IMPLEMENTATION NOTE (targeted re-audit of 2466e03, P1 and its P2 sibling): the frozen TITLE also requires that THIS attempt READ that no row stamped for this claim exists. triggerClaimRefund tracks it (ownRowAbsent: set by each stamped read, cleared when a stamped row is seen or a stamped read fails, false before the first read) and passes it to alertClaimPaymentBlocked, where it decides the title only (facts unchanged). So an attempt_crashed after the own-row write threw, and a safety_check_unreadable from an unreadable stamped read at (a) or (f), carry the outcome-unknown title; callers outside the trigger keep the frozen title on Stripe evidence for the bound row. Break/restore controls R25, R26, R27.
IMPLEMENTATION NOTE (targeted re-audit of d9fb194, P1 and P3): « cleared when a stamped row is seen » now covers the loader's read of the order's rows (C3 note): a row stamped for this claim seen only by the loader takes own_row_exists, so no hold, window revert or lock proof carries the frozen title with such a row read. Outside the trigger, the frozen title rests on: applyRowTruth and R0 — row or Stripe evidence for the bound row; N8 — its re-read of the rows stamped for the claim immediately before its CAS; refunds_disabled — triggerClaimRefund returned before T1, so this request made no engine call (a row left by an earlier stalled attempt is the declared C11 / A-S33 residual).
IMPLEMENTATION NOTE (targeted re-audit of 75f1601, P1): the d9fb194 note above (« no hold, window revert or lock proof carries the frozen title with such a row read ») did not hold when the loader failed after reading the rows. Inside triggerClaimRefund the title now follows the stamped read each non-engine exit takes right before its write (C3 note): a row found → own_row_exists; a failed read → the outcome-unknown title; a read that returns none → the frozen title, true even after an earlier failed read, since refund rows are never deleted.
IMPLEMENTATION NOTE (targeted re-audit of 68621aa): the park's own read (C3 note) also decides the title of a crash in the park's compare-and-set: the catch's attempt_crashed title rests on that read. After a failed own read in a hold, a proof or the park, the title is set by the revert's own read (frozen when it returns none, outcome-unknown when it fails).
...
IMPLEMENTATION NOTE (targeted re-audit of 68621aa, P3): C3's stamped-read absence proof rests on Refund rows never being deleted or re-stamped. The source scan now also asserts that app/, lib/ and scripts/ contain no refund.delete or refund.deleteMany call, no delete through a dynamic model accessor (tx[model].delete / deleteMany) whose model resolves to refund or cannot be resolved, and no raw SQL DELETE FROM or UPDATE on the Refund table, each shape with a negative control. The only dynamic-accessor deletes are the Clean Room script's delByIds / wipe helpers, called with literal models, none of them refund; the script's header also excludes Refund and Claim from every delete.
...
IMPLEMENTATION NOTE (certification audit c32d8d3): the title assertion follows the I-01 note — CLAIM_BLOCKED_TITLE where engineCalled is false and the cause is not own_row_exists, CLAIM_BLOCKED_OUTCOME_UNKNOWN_TITLE otherwise — with a real engine_own_row alert and a negative control (the frozen title on an engineCalled alert is red).
IMPLEMENTATION NOTE (targeted re-audit of 2466e03): the expected title comes from each fixture's declared outcome, never from the alert's own facts; fixtures added for the own-row write that throws after a stamped row was read, and for an unreadable stamped read at (a) and at (f).
IMPLEMENTATION NOTE (targeted re-audit of d9fb194): read-skew fixtures added for a row stamped claim:cl1 seen only by the loader on the (e') window-revert and (c) hold paths — both now own_row_exists with the outcome-unknown title and the own-row text, engine not called.
IMPLEMENTATION NOTE (targeted re-audit of 75f1601): fixtures added in tests/claims-t49-round13-alerts.test.ts — the loader failing after its row read with a row stamped claim:cl1 (the PaymentIntent retrieve, the royalty read, that row's Stripe refund, list_over_cap); a row stamped claim:cl1 inserted after the loader, before the (b) revert, the (b') and (c) holds, the (e') window revert and the (e') lock proof; the no_charge arm of the loader check (pinned by the stamped-read count); the (f) failure followed by a read that returns none (frozen title) or that fails too (outcome-unknown title).
IMPLEMENTATION NOTE (targeted re-audit of 68621aa): fixtures added in tests/claims-t49-round13-alerts.test.ts — FAILED_OWN_READ (five exits × the exit's own read failing, then the revert's read returning none or failing too: safety_check_unreadable, the claim back at its pre-image, no hold, proof or park text, no financial-verification alert, one alert with the frozen or the outcome-unknown title) and the (e') park in INSERTED_AFTER_LOADER (own_row_exists, two stamped reads, no park).
...
IMPLEMENTATION NOTE (certification audit c32d8d3): the title follows the I-01 note (claimBlockedTitle), pinned in tests/claims-t49-round13-alerts.test.ts.
...
IMPLEMENTATION NOTE (certification audit c32d8d3, P1): (b) with the helper failing is pinned — a failed row (redelivery) whose marking throws answers 503 {received:false}, leaves the claim unchanged, and the next delivery marks it exactly once (tests/claims-t49-round13-reversal.test.ts); break/restore control R22.
