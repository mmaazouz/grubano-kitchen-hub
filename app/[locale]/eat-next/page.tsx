...
export default async function EatNextHome() {
  const t = await getTranslations('eatNext.home')
  ...
  return (
    <div className="space-y-4 p-4">
      <div className="flex items-center gap-2 text-sm text-stellar-muted-fg">
        ...
        {t('deliverTo')} <span className="font-semibold text-stellar-fg">{t('address')}</span> ›
      </div>
      ...
      <Link href="/eat-next/search" className="block rounded-stellar-xl border border-stellar-border bg-stellar-surface-1 px-4 py-3 text-stellar-muted-fg shadow-stellar-soft">
        🔎 <span className="font-stellar-display font-semibold text-stellar-fg">{t('searchEntry')}</span> <span className="text-stellar-muted-fg">{t('searchEntryTail')}</span>
      </Link>
      ...
      <h2 className="pt-2 font-stellar-display text-sm font-bold uppercase tracking-wide text-stellar-muted-fg">{t('nearby')}</h2>
            ...
            <li key={r.id}>
              <Link href={`/eat-next/r/${r.id}`} className="block">
                ...
                <p className="mt-1 px-1 text-xs text-stellar-muted-fg">
                  {t('minimum')}&nbsp;: <StellarPriceTag amountEur={r.minOrderEur} size="sm" muted /> · {t('reviews', { count: r.reviewCount })}
                </p>
              </Link>
            </li>
          ))}
