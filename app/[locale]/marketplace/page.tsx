...
// HONEST: the delivery / payment / marketing "integrations" have NO connector rail
// yet, so they are listed inert with a « bientôt » badge — NO invented commission /
// rating / "Connecté" state and no plan-specific upsell (the previous emoji + fake
// 25%/4.8★/Lydia=connected demo data is gone). The two REAL destinations keep their
// links: supplier sourcing (always) and services discovery (flag-gated, hidden when
// OFF — no leak). ZERO money on this screen.

const integrations = [
  { name: 'UberEats',  cat: 'Delivery' },
  { name: 'Deliveroo', cat: 'Delivery' },
  { name: 'Just Eat',  cat: 'Delivery' },
          ...
          {integrations.map((i) => (
            <div key={i.name} className="set-row is-inert">
              ...
              <span className="set-row__m">
                <b>{i.name}</b>
                <span>
                  {i.cat === 'Delivery' ? t('hubCatDelivery') : i.cat === 'Payment' ? t('hubCatPayment') : t('hubCatMarketing')}
                </span>
              </span>
