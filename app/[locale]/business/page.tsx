...
import {
  ChefHat, ArrowRight, LogIn, Check, BadgeCheck, ShieldCheck, Layers,
...
// Marketing vitrine: hero + product mockup, trust band, trades, how-it-works,
// benefits, final CTA, footer. PURE PRESENTATION. Routes to existing journeys
// ([Devenir partenaire] → /business/start, [Se connecter] → /auth/magic). The
// internal pages keep their own PartnerChrome; this landing has its own richer
// header/footer. Reduced-motion safe (motion-reduce CSS net + IntersectionObserver).
...
export default function BusinessLandingPage() {
  ...
  const reassure = [t('reassure1'), t('reassure2'), t('reassure3')]
    ...
    <div className="min-h-screen bg-white text-grubano-ink">
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-3.5">
          <Link href="/business" className="flex items-center gap-2.5" aria-label="Grubano">
            ...
            <span className="leading-none">
              <span className="block font-display text-base font-extrabold text-grubano-ink">Grubano</span>
      ...
      <section className="relative overflow-hidden">
        ...
        <div aria-hidden className="pointer-events-none absolute inset-0">
          <div className="absolute -top-32 end-[-10%] h-[480px] w-[480px] rounded-full bg-grubano-primary/10 blur-3xl" />
          <div
            className="absolute inset-0 opacity-[0.5]"
            style={{
              backgroundImage: 'radial-gradient(circle, rgba(20,20,20,0.05) 1px, transparent 1px)',
        ...
        <div className="mx-auto grid max-w-6xl items-center gap-12 px-5 pb-16 pt-14 md:pt-20 lg:grid-cols-2 lg:gap-10 lg:pb-24">
          <div className="motion-safe:animate-slide-up">
            ...
            <h1
              className="mt-4 font-display font-extrabold tracking-tight text-grubano-ink"
              style={{ fontSize: 'clamp(2.4rem, 5.2vw, 4.25rem)', lineHeight: 1.05 }}
            >
              {t('heroTitle')}
            </h1>
            ...
            <ul className="mt-7 flex flex-wrap items-center gap-x-5 gap-y-2 text-grubano-sm font-medium text-grubano-ink-muted">
              {reassure.map((r, i) => (
                <li key={i} className="inline-flex items-center gap-1.5">
                  <Check size={15} className="text-grubano-primary" /> {r}
                </li>
              ))}
            </ul>
          </div>
          ...
          <div className="motion-safe:animate-fade-in lg:justify-self-end" style={{ animationDelay: '120ms' }}>
            <HeroMockup />
          </div>
        </div>
      </section>
      ...
      {/* ── Trust band ── */}
      <section className="border-y border-grubano-border bg-grubano-surface-muted/50">
        <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-center gap-x-8 gap-y-3 px-5 py-5 text-grubano-sm font-semibold text-grubano-ink-muted">
          {[{ i: <BadgeCheck size={16} />, t: t('reassure1') }, { i: <ShieldCheck size={16} />, t: t('reassure2') }, { i: <Layers size={16} />, t: t('reassure3') }].map((x, i) => (
            <span key={i} className="inline-flex items-center gap-2"><span className="text-grubano-primary">{x.i}</span>{x.t}</span>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-5 py-24 md:py-28">
        <Reveal className="mx-auto max-w-2xl text-center">
          <h2 className="font-display text-grubano-3xl font-extrabold tracking-tight text-grubano-ink sm:text-grubano-4xl">{t('tradesTitle')}</h2>
      ...
      <section className="border-y border-grubano-border bg-grubano-surface-muted/40">
        <div className="mx-auto max-w-5xl px-5 py-24 md:py-28">
          <Reveal className="mx-auto max-w-2xl text-center">
            <h2 className="font-display text-grubano-3xl font-extrabold tracking-tight text-grubano-ink sm:text-grubano-4xl">{t('howTitle')}</h2>
          ...
          <div className="mt-14 grid grid-cols-1 gap-8 sm:grid-cols-3">
            {steps.map((s, i) => (
              <Reveal key={i} delay={i * 90} className="relative text-center">
                <div className="mx-auto grid h-12 w-12 place-items-center rounded-grubano-pill bg-grubano-dark font-display text-grubano-lg font-extrabold text-grubano-primary shadow-grubano-sm">
                  {i + 1}
                </div>
      ...
      <section className="mx-auto max-w-6xl px-5 py-24 md:py-28">
        <Reveal className="mx-auto max-w-2xl text-center">
          <h2 className="font-display text-grubano-3xl font-extrabold tracking-tight text-grubano-ink sm:text-grubano-4xl">{t('benefitsTitle')}</h2>
        </Reveal>
        <div className="mt-12 grid grid-cols-1 gap-5 md:grid-cols-3">
          {benefits.map((b, i) => (
            <Reveal key={i} delay={i * 80}>
              <div className="flex h-full flex-col gap-3 rounded-grubano-xl border border-grubano-border bg-white p-6 shadow-grubano-sm">
                <span className="grid h-11 w-11 place-items-center rounded-grubano-md bg-grubano-tint text-grubano-primary">
                  <b.icon size={20} />
                </span>
      ...
      <section className="px-5 pb-24">
        <Reveal className="mx-auto max-w-4xl">
          <div className="relative overflow-hidden rounded-grubano-2xl border border-grubano-primary/20 bg-gradient-to-br from-grubano-tint via-white to-grubano-tint/50 px-6 py-12 text-center shadow-grubano-md sm:px-12 sm:py-16">
            <h2 className="font-display text-grubano-2xl font-extrabold tracking-tight text-grubano-ink sm:text-grubano-3xl">{t('finalTitle')}</h2>
          ...
          <div className="flex flex-col items-start justify-between gap-6 sm:flex-row sm:items-center">
            <div className="max-w-xs">
              <div className="flex items-center gap-2.5">
                ...
                <span className="font-display text-grubano-base font-extrabold text-grubano-ink">Grubano</span>
              </div>
...
function HeroMockup() {
  ...
  return (
    <div className="relative w-full max-w-md">
      ...
      <div className="absolute -end-3 -top-3 z-10 hidden items-center gap-1.5 rounded-grubano-pill bg-white px-3 py-1.5 text-grubano-xs font-bold text-grubano-success shadow-grubano-lg sm:inline-flex">
        <BadgeCheck size={14} /> {t('reassure1')}
      </div>

      <div className="overflow-hidden rounded-grubano-xl border border-grubano-border bg-white shadow-grubano-premium">
        {/* browser chrome */}
        <div className="flex items-center gap-2 border-b border-grubano-border bg-grubano-surface-muted/70 px-3 py-2.5">
          ...
          <span className="mx-auto flex items-center gap-1.5 rounded-grubano-pill bg-white px-3 py-1 text-[11px] font-medium text-grubano-ink-faint shadow-grubano-sm">
            <ShieldCheck size={11} className="text-grubano-success" /> {t('mockUrl')}
          </span>
        </div>
...
function MetricCard({ icon, label, value, trend }: { icon: React.ReactNode; label: string; value: string; trend: string }) {
  return (
    <div className="rounded-grubano-md border border-grubano-border bg-white p-3">
      ...
      <p className="mt-1.5 font-display text-grubano-xl font-extrabold text-grubano-ink">{value}</p>
