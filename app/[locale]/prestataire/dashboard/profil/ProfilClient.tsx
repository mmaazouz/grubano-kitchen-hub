...
import { Wrench, ArrowLeft, CheckCircle2, Lock, Loader2 } from 'lucide-react'
import { Link } from '@/navigation'
import { Card, Button, Input } from '@/components/design-system'

// ── /prestataire/dashboard/profil — editable prestataire Profile (Agent 104) ──
// Client form: GET /api/prestataire/profile to pre-fill, PATCH to save the OPERATIONAL fields
// only. Identity fields (SIREN / official name / verification / account status) are shown
// READ-ONLY and are never sent — the API whitelists operational fields and ignores the rest.
// Faithful calque of /supplier/dashboard/profil, adapted to the prestataire field set
// (serviceCategories / coverageZones / modality / indicativeRate). Same sober bare chrome as
// /prestataire/dashboard. NO money — the prestataire payment rail is untouched.
...
// Enum → i18n svc* label key (camelCased), mirroring the dashboard's SVC_LABEL map.
...
type ProfileData = {
  ...
  serviceCategories: string[]; coverageZones: string[]; modality: string; indicativeRate: string
...
export default function ProfilClient() {
  ...
  const [modality, setModality] = useState<string>('on_site')
  ...
  useEffect(() => {
    let cancelled = false
    fetch('/api/prestataire/profile', { cache: 'no-store' })
      ...
      .then((d) => {
        ...
        setPhone(p.phone ?? '')
        setCity(p.city ?? '')
        setCategories(Array.isArray(p.serviceCategories) ? p.serviceCategories : [])
        setZones((p.coverageZones ?? []).join(', '))
        setModality(p.modality ?? 'on_site')
        setRate(p.indicativeRate ?? '')
  ...
  // Clear the "saved" banner as soon as the user edits again (it reflected the previously-saved
  // state, not the now-dirty form). No-op during the prefill (saved is false then).
  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(() => { if (saved) setSaved(false) }, [companyName, contactName, phone, city, categories, zones, modality, rate])
  ...
  const statusLabel = (s: string) =>
    s === 'active' ? t('dashboard.statusActive')
    : s === 'suspended' ? t('dashboard.statusSuspended')
    : s === 'rejected' ? t('dashboard.statusRejected')
  ...
  return (
    <div className="min-h-screen bg-grubano-bg">
      ...
      <main className="mx-auto max-w-3xl space-y-5 px-5 py-6">
        <Link href="/prestataire/dashboard" className="inline-flex items-center gap-1.5 text-grubano-sm font-semibold text-grubano-ink-muted hover:text-grubano-primary">
          ...
          <>
            ...
            <form onSubmit={save} className="space-y-5">
              <Card elevation="sm" padding="lg" className="space-y-4">
                ...
                <div>
                  <p className="mb-1.5 text-grubano-sm font-medium text-grubano-ink">{t('fieldServices')}</p>
                ...
                <div>
                  <p className="mb-1.5 text-grubano-sm font-medium text-grubano-ink">{t('fieldModality')}</p>
                  <div className="flex flex-wrap gap-2">
                    {MODALITIES.map((m) => {
                ...
                <div>
                  <Input label={t('fieldCoverageZones')} value={zones} onChange={(e) => setZones(e.target.value)} placeholder="Lyon, Villeurbanne, 69003" />
                  <p className="mt-1 text-grubano-xs text-grubano-ink-faint">{t('fieldCoverageZonesHint')}</p>
                </div>

                <div>
                  <label className="mb-1.5 block text-grubano-sm font-medium text-grubano-ink">{t('fieldIndicativeRate')}</label>
                  <textarea
                    value={rate}
                    onChange={(e) => setRate(e.target.value)}
                    rows={2}
                    maxLength={500}
                    placeholder={t('fieldIndicativeRatePlaceholder')}
                  ...
                  />
                  <p className="mt-1 text-grubano-xs text-grubano-ink-faint">{t('fieldIndicativeRateHint')}</p>
                </div>
              ...
              {/* Locked identity block — read-only, never editable (SIREN/KYB are set at registration). */}
              {locked && (
