...
export async function GET() {
  ...
  const op = await prisma.operator.findUnique({ where: { id: userId }, select: { name: true, phone: true } })
  if (!op) return NextResponse.json({ error: 'not_found' }, { status: 404 })
  return NextResponse.json({ name: op.name, phone: op.phone ?? '' })
}

// name is required-non-empty WHEN provided (Operator.name is non-null); phone is free-form
// and an empty string CLEARS it. An absent field is left untouched.
...
export async function PATCH(req: Request) {
  ...
  const op = await prisma.operator.update({ where: { id: userId }, data, select: { name: true, phone: true } })
  return NextResponse.json({ name: op.name, phone: op.phone ?? '' })
}
