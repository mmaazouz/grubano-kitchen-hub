...
import { LayoutGrid, Check, Plus } from 'lucide-react'
...
export default function RoleSwitcher({ tone = 'dark' }: { tone?: 'dark' | 'light' }) {
  ...
  // Anonymous / not loaded yet → nothing. A CONNECTED user (≥1 space) always sees
  // the "add an activity" entry below — even a mono-role one (the whole point: a
  // restaurateur with a single space should be able to add a second activity).
  if (spaces.length === 0) return null

  const current = currentSpace(pathname, spaces)
  const dark = tone === 'dark'
  const multi = spaces.length > 1
  const addActive = pathname === '/add-activity' || pathname.startsWith('/add-activity/')
  ...
  return (
    <div className={cn('px-3 py-3', dark && 'border-b border-white/10')}>
      {/* Space switcher — only when there is more than one space to switch between. */}
      {multi && (
        <>
          <p className={cn(
            ...
            {spaces.map((sp) => {
              ...
              return (
                <Link
                  ...
                  className={cn('justify-between', itemClass(active))}
                >
      ...
      {/* "Add an activity" — always available to a connected user. */}
      <Link
        href="/add-activity"
        aria-current={addActive ? 'page' : undefined}
        className={cn(multi && 'mt-0.5', itemClass(addActive))}
      >
